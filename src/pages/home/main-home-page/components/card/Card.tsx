...
const Card: React.FC = () => {
  ...
  // array.map-is Chveneburi versia!!!!!
  ...
  // }
  // const nums = [1, 2, 3];
  // const foo = myMapFunc(nums, (el: number) => {
  //   return el + 3;
  // });
  // console.log(foo);
  ...
  const handleVote = (id: string) => {
    setCountries((prevCountries) =>
      prevCountries.map((country) =>
        country.id === id ? { ...country, votes: country.votes + 1 } : country
      )
    );
  };
  // sorting my countries by vote (zrdadobiT)
  const sortIncreasing = () => {
    const sortedCountries = [...countries].sort((a, b) => a.votes - b.votes);
